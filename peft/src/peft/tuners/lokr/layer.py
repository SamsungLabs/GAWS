    ...
    def update_layer(
        ...
        **kwargs,
        ...
        if isinstance(base_layer, nn.Linear) :
            ...
            in_dim, out_dim = tuple(base_layer.weight.shape)
            in_m, in_n = factorization(in_dim, decompose_factor)
            out_l, out_k = factorization(out_dim, decompose_factor)
            shape = ((out_l, out_k), (in_m, in_n))  # ((a, b), (c, d)), out_dim = a*c, in_dim = b*d
...
class Linear(LoKrLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.is_conv1d = isinstance(self.get_base_layer(), Conv1D)
    ...
    def _get_delta_activations(
        ...
        return F.linear(input, delta_weight.T if self.is_conv1d else delta_weight)

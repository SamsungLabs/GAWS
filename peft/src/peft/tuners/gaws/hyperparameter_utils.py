...
def get_valid_split_dim_values(model, target_module):
...
def get_factors(in_features, out_features):
    ...
    for factor in range(2, in_features):
        if in_features % factor ==  0:
            input_splits.append(factor)
            input_split_dim.append(int(in_features /factor))
            output_split_dim.append(int(const*in_features /factor))

    df_dic = {
        "input_split_dim": input_split_dim,
        "output_split_dim": output_split_dim,
        "input_splits = output_splits": input_splits,
    }

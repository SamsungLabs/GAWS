...
def get_peft_model_state_dict(
    ...
    elif config.peft_type == PeftType.GAWS:
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "GAWS" in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if "GAWS" in k or "bias" in k}
        elif bias == "gaws_only":
            to_return = {}
            for k in state_dict:
                if "GAWS" in k:
                    to_return[k] = state_dict[k]
                    bias_name = k.split("GAWS")[0] + "bias"
                    if bias_name in state_dict:
                        to_return[bias_name] = state_dict[bias_name]
        else:
            raise NotImplementedError
        to_return = {k: v for k, v in to_return.items() if (("GAWS" in k and adapter_name in k) or ("bias" in k))}
        
    elif config.peft_type == PeftType.GAWSD:
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "GAWSD" in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if "GAWSD" in k or "bias" in k}
        elif bias == "gaws_d_only":
            to_return = {}
            for k in state_dict:
                if "GAWSD" in k:
                    to_return[k] = state_dict[k]
                    bias_name = k.split("GAWSD")[0] + "bias"
                    if bias_name in state_dict:
                        to_return[bias_name] = state_dict[bias_name]
        else:
            raise NotImplementedError
        to_return = {k: v for k, v in to_return.items() if (("GAWSD" in k and adapter_name in k) or ("bias" in k))}
